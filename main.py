...
def gen_markup(rows):
    markup = ReplyKeyboardMarkup(one_time_keyboard=True)  # Добавлено one_time_keyboard=True
    markup.row_width = 1
    for row in rows:
...
def callback_project(message, data, statuses):
    ...
    bot.send_message(message.chat.id, "Введите ссылку на фото проекта (опционально)")
...
def photo_project(message, data):
    ...
    if message.text == cancel_button:
        ...
        return
    data.append(photo)
    manager.insert_project(data[0], data[1], data[2], data[3], data[4], data[5])
